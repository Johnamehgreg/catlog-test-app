...
import {
  interBoldFontFamily,
  interMediumFontFamily,
  interRegularFontFamily,
  textStyles,
...
export const ExpenseCardItem: React.FC<{ expense: ExpenseModel }> = ({
  expense,
}) => {
  const { isDark } = useDarkMode();
  return (
    <>
      <View className="app-border app-card " style={[styles.card]}>
        <HStack className="justify-between items-start">
          <HStack style={{ gap: s(12) }} className="flex-1 items-center">
            ...
            >
              <Image
                source={require('@assets/pngs/receipt.png')}
          ...
          <TouchableOpacity
            style={{
              padding: s(5),
              backgroundColor: isDark ? '#1f2937f7' : '#F8F8F8',
              borderRadius: s(20),
            }}
          >
            <MoreHorizontal
